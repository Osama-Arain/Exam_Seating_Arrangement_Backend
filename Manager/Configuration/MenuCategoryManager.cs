...
namespace API.Manager.Configuration
{
    public class MenuCategoryManager : IManager
    {
        ...
        public async Task<ApiResponse> GetDataAsync(ClaimsPrincipal _User)
        {
            ...
            try {
                var _Table = await _context.MenuCategories.Include(x => x.Tenants).Where (a =>  a.Action != Enums.Operations.D.ToString ()).OrderBy (o => o.Name).ToListAsync ();
        ...
        public async Task<ApiResponse> GetDataByIdAsync(Guid _Id,ClaimsPrincipal _User)
        {
            var apiResponse = new ApiResponse ();
            try {
                var _Table = await _context.MenuCategories.Include(x => x.Tenants).Where (a => a.Id == _Id && a.Action != Enums.Operations.D.ToString ()).FirstOrDefaultAsync ();
        ...
        public async Task<ApiResponse> UpdateAsync(object model, ClaimsPrincipal _User)
        {
            ...
            try {
                ...
                result.TenantId = _model.TenantId;
